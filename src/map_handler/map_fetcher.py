...
def crop_image(im : Image, scale_factor : float) -> Image:
    ...
    (left, upper, right, lower) = (width_to_crop//2, height_to_crop//2, width - (width_to_crop//2), height - (height_to_crop//2))
...
def add_centered_text(text : str, height : int, im : Image, font_size : int = 24, font_weight : float = 1):
    draw = ImageDraw.Draw(im)
    draw.text(xy=(PAGE_WIDTH//2, height),
              text=text,
              anchor="mm",
              fill=FG_COLOR_RGB,
